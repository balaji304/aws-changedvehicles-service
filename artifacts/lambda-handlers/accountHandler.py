...
class HeplerImporter(Helper):
    def get_vehicleids(self, eventData, sourceObj):
        ...
        scan_result = self.scan_dynamo_tableindex_with_filterExp(f"vehicle-{STAGE}-EntityDB", "key-Id-index", filterExp)
    ...
    def scan_dynamo_tableindex_with_filterExp(self, tableName, indexName, filterExp):
        table = self.dynamoResourceClient.Table(f'vehicle-{STAGE}-EntityDB')
        response = table.scan(
...
if __name__ == "__main__":
    SourceSqsUrl = 'https://sqs.eu-central-1.amazonaws.com/831745121203/Account-TopicSubscriber-staging-changedvehicles-service-Queue'
